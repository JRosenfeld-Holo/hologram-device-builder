...
export default function BOMTable({ items, title }: BOMTableProps) {
  ...
  return (
    <div className="rounded-xl border border-[#3A3C46]/45 overflow-hidden">
        ...
        <div className="bg-[#030710]">
          {items.map((item, idx) => (
            <div key={idx} className="border-b border-[#3A3C46]/20 last:border-b-0">
              <div
                className={`grid grid-cols-[1fr_1.5fr_2fr_auto] gap-0 items-center hover:bg-white/[0.015] transition-colors duration-100 ${item.detail ? "cursor-pointer" : ""
                  }`}
      ...
      <div className="sm:hidden bg-[#030710] divide-y divide-[#3A3C46]/20">
        {items.map((item, idx) => (
          <div
            key={idx}
            className={`p-4 ${item.detail ? "cursor-pointer" : ""}`}
            onClick={() => item.detail && setExpanded(expanded === idx ? null : idx)}
          >
            {/* Category + Cost row */}
            <div className="flex items-center justify-between mb-2">
              <p className="font-mono text-[10px] font-semibold tracking-widest uppercase text-[#BFFD11]/70">
                {item.category}
              </p>
              <div className="flex items-center gap-2">
                {item.unitCost && (
                  <span className="text-xs text-white/35 font-mono">{item.unitCost}</span>
                )}
                {item.detail && (
                  <motion.div
                    animate={{ rotate: expanded === idx ? 180 : 0 }}
            ...
            {/* Part name */}
            <p className="text-sm font-medium text-white/85 mb-1">{item.part}</p>
            {item.alternatives && (
              <p className="text-xs text-white/30 mb-2">Alt: {item.alternatives}</p>
            )}

            {/* Rationale */}
            <p className="text-xs text-white/45 leading-relaxed">{item.rationale}</p>
            ...
            <AnimatePresence initial={false}>
              {expanded === idx && item.detail && (
                <motion.div
                  ...
                  className="overflow-hidden"
                >
                  <div className="mt-3 pt-3 border-t border-[#BFFD11]/10 bg-[#BFFD11]/3 -mx-4 px-4 pb-1 rounded-b-lg">
